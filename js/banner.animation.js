...
Banner.prototype.start = function () {
  ...
  this.images = ['images/logo.png', 'images/bg.jpg', 'images/copy.png', 'images/cta.png', 'images/e-cover.png', 'images/image.jpg', 'images/legal.png', 'images/logo.png', 'images/seats.png', 'images/price.png', 'images/title.png'];
...
Banner.prototype.setupFlip = function (front, back) {
  var timeline = new TimelineMax({ paused: true }).addLabel('start').set(front, { autoAlpha: 1 }).set(back, { autoAlpha: 1, rotationX: -90 }).to(front, 0.3, { rotationX: 90, ease: Linear.easeNone }, 'hide-front').to(back, 0.3, { rotationX: 0, ease: Linear.easeNone }, 'show-back');
...
Banner.prototype.animate = function () {
  ...
  this.timeline = new TimelineLite({ paused: false }).to([this.eCover, this.bg], 1, { x: '+=300' }, '+=2').from(this.copy, 1, { autoAlpha: 0 }, '-=1').to([this.bg, this.eCover], 1.5, { x: '-=600' }, 'yellow+=1').from(this.logo, 1.5, { autoAlpha: 0, x: '+=600' }, 'yellow+=1').from(this.title, 1, { autoAlpha: 0 }).from(this.price, 1, { autoAlpha: 0 }).from(this.cta, 1, { autoAlpha: 0 }, 'cta-=0.5').from(this.legal, 1, { autoAlpha: 0 }, 'cta-=0.5').add(this.setupFlip(this.price, this.seats).play(), '+=1').add(this.setupFlip(this.seats, this.price).play(), '+=2');
};